...
rm(list=ls())
# load libraries
library(kableExtra); library(ez); library(effsize); library(knitr); library(RColorBrewer); library(tidyverse)
...
money.inc.means = group_by(feedback.merged, Subject, moneyreward) %>% filter(!is.na(subRewarded)) %>%
  dplyr::summarise(n=n(), meanRR = mean(subRewarded), meanACC = mean(Target.ACC), meanErr = mean(subError))
...
t.test(x = subset(incentive.means, IncentiveType=="Social")$meanRR,
       ...
       paired = TRUE)
cohen.d(d = subset(incentive.means, IncentiveType=="Social")$meanRR,
       f = subset(incentive.means, IncentiveType=="Liquid")$meanRR,
       paired = TRUE)
...
t.test(x = subset(val.inc.means, IncentiveType=="Liquid" & Valence=="Positive")$meanRR,
       y = subset(val.inc.means, IncentiveType=="Liquid" & Valence =="Negative")$meanRR,
       paired = TRUE)
cohen.d(d = subset(val.inc.means, IncentiveType=="Liquid" & Valence=="Positive")$meanRR,
       f = subset(val.inc.means, IncentiveType=="Liquid" & Valence =="Negative")$meanRR,
       paired = TRUE)
...
t.test(x = subset(incentive.means, IncentiveType=="Social")$meanACC,
       ...
       paired = TRUE)
cohen.d(d = subset(incentive.means, IncentiveType=="Social")$meanACC,
       f = subset(incentive.means, IncentiveType=="Liquid")$meanACC,
       paired = TRUE)
...
t.test(x = subset(incentive.means, IncentiveType=="Social")$meanErr,
       ...
       paired = TRUE)
cohen.d(d = subset(incentive.means, IncentiveType=="Social")$meanErr,
       f = subset(incentive.means, IncentiveType=="Liquid")$meanErr,
       paired = TRUE)
...
###Summary Table of Reward Rate, Accuracy, Error Rate, and RT

``` {r RRSum, message = F, warning =  F}
#Summary Table of Rewrard Rate, Accuracy, Error Rate, and RT
RR.summary<-summarySEwithin2(data = val.inc.mon.means, measurevar = "meanRR", withinvars = c("Valence","IncentiveType","moneyreward"), idvar = "Subject")
...
summary.table <- cbind(RR.summary,ACC.sum, Err.sum, RT.sum) %>%
  rename("RR_mean" = meanRR,
         "RR_sd" = sd) %>%
  select(-c("N","meanRRNormed","se","ci"))

###Summary Table: Reward Rate, Accuracy, Error Rate, RT
...
tab.RR.sum <- summary.table %>% 
  ...
  kable(., format = "html", digits = 3, booktabs = T,
        col.names = c("Feedback Valence", "Monetary Reward", rep(c("Mean","SD"), times = 8)),
...
p.RR.Exp1<-ggplot(RR.sum, aes(x=Valence, y=meanRR, fill=moneyreward)) + 
  ...
  labs(x="Feedback Valence",y="Reward Rate") +
...
d.anova.E1 <- omnibus.anova.E1$ANOVA %>%
  mutate(Effect = factor(Effect, levels = c("Valence","IncentiveType","moneyreward","Valence:IncentiveType",
                         ...
                         labels = c("Feedback Valence","Incentive Type","Monetary Reward","Feedback Valence x Incentive Type","Feedback Valence x Monetary Reward",
                                    "Incentive Type x Monetary Reward", "Feedback Valence x Incentive Type x Monetary Reward"))) %>%
...
d.soc.anova.E1 <- social.anova.E1$ANOVA %>%
  mutate(Effect = factor(Effect, levels = c("Valence","moneyreward","Valence:moneyreward"),
                         labels = c("Feedback Valence","Monetary Reward","Feedback Valence x Monetary Reward"))) %>%
  rename("Generalized Eta Squared" = ges)
tab.soc.E1 <- kable(d.soc.anova.E1, digits = 3, caption = "Social Feedback ANOVA (Feedback Valence x Monetary Reward)") %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
tab.soc.E1
...
d.liq.anova.E1 <-liquid.anova.E1$ANOVA %>%
  mutate(Effect = factor(Effect, levels = c("Valence","moneyreward","Valence:moneyreward"),
                         labels = c("Feedback Valence","Monetary Reward","Feedback Valence x Monetary Reward"))) %>%
  rename("Generalized Eta Squared" = ges)
tab.liq.E1 <- kable(d.liq.anova.E1, digits = 3, caption = "Liquid Feedback ANOVA (Feedback Valence x Monetary Reward)") %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
tab.liq.E1
```
...
p.affect.valence <- ggplot(word.means.valence.sum, aes(x=Word_Valence, y=meanRating, fill=Valence)) +
  ...
  geom_errorbar(position=position_dodge(width=0.9), aes(ymin=meanRating-ci, ymax=meanRating+ci), width=.2) + 
   scale_fill_brewer(palette="Set1", name="Feedback Valence", breaks=c("Positive", "Neutral", "Negative"), labels=c("Positive", "Neutral", "Negative")) +
  xlab("") + ylab("Affect Rating") +
  scale_x_discrete(labels=c("Positive" = "Positive\nTerms", "Negative" = "Negative\nTerms")) +
  facet_grid(.~IncentiveType)
p.affect.valence
```
...
tab.affect.sum <- word.means.valence.sum %>% select(Valence, Word_Valence, IncentiveType, meanRating, sd) %>%
  ...
  kable(., format = "html", digits = 3, booktabs = T,
        col.names = c("Feedback Valence", "Affect Term Valence", rep(c("Mean","SD"), times = 2)),
...
###ANOVA Tables: Incentive Type x Affect Term Valence x Feedback Valence
...
affect.anova.E1 <- affect.anova$ANOVA %>%
    mutate(Effect = factor(Effect, levels = c("Word_Valence","Valence","IncentiveType","Word_Valence:Valence","Word_Valence:IncentiveType",
            ...
            labels = c("Affect Term Valence","Feedback Valence","Incentive Type", "Affect Term Valence x Feedback Valence",
                      "Affect Term Valence x Incentive Type", "Feedback Valence x Incentive Type","Affect Term Valence x Feedback Valence x Incentive Type"))) %>%
  rename("Generalized Eta Squared" = ges)
tab.affect <- kable(affect.anova.E1, digits = 3, caption = "Omnibus ANOVA (Incentive Type x Word Valence x Feedback Valence)") %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
tab.affect
...
affect.soc.anova.E1 <- social.affect.anova$ANOVA %>%
  mutate(Effect = factor(Effect, levels = c("Word_Valence","Valence","Word_Valence:Valence"),
                         labels = c("Affect Term Valence","Feedback Valence","Affect Term Valence x Feedback Valence"))) %>%
  rename("Generalized Eta Squared" = ges)
tab.soc.affect <- kable(affect.soc.anova.E1, digits = 3, caption = "Social Feedback ANOVA (Word Valence x Feedback Valence)") %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
tab.soc.affect

t.test(x = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Positive")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Negative")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Positive")$meanRating,
       f = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Negative")$meanRating,
       paired = TRUE)
t.test(x = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Positive")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Neutral")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Positive")$meanRating,
       f = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Neutral")$meanRating,
       paired = TRUE)
t.test(x = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Neutral")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Negative")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Neutral")$meanRating,
       f = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Positive" & Valence=="Negative")$meanRating,
       paired = TRUE)


t.test(x = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Positive")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Negative")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Positive")$meanRating,
       f = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Negative")$meanRating,
       paired = TRUE)
t.test(x = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Positive")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Neutral")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Positive")$meanRating,
       f = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Neutral")$meanRating,
       paired = TRUE)
t.test(x = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Neutral")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Negative")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Neutral")$meanRating,
       f = subset(word.means.valence, IncentiveType=="Social" & Word_Valence=="Negative" & Valence=="Negative")$meanRating,
       paired = TRUE)
...
affect.liq.anova.E1 <-liquid.affect.anova$ANOVA %>%
    mutate(Effect = factor(Effect, levels = c("Word_Valence","Valence","Word_Valence:Valence"),
                         labels = c("Affect Term Valence","Feedback Valence","Affect Term Valence x Feedback Valence"))) %>%
  rename("Generalized Eta Squared" = ges)
tab.liq.affect <- kable(affect.liq.anova.E1, digits = 3, caption = "Liquid Feedback ANOVA (Word Valence x Feedback Valence)") %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
tab.liq.affect

t.test(x = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Positive")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Negative")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Positive")$meanRating,
       f = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Negative")$meanRating,
       paired = TRUE)
t.test(x = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Positive")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Neutral")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Positive")$meanRating,
       f = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Neutral")$meanRating,
       paired = TRUE)
t.test(x = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Neutral")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Negative")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Neutral")$meanRating,
       f = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Positive" & Valence=="Negative")$meanRating,
       paired = TRUE)


t.test(x = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Positive")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Negative")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Positive")$meanRating,
      f = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Negative")$meanRating,
       paired = TRUE)
t.test(x = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Positive")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Neutral")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Positive")$meanRating,
       f = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Neutral")$meanRating,
       paired = TRUE)
t.test(x = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Neutral")$meanRating,
       y = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Negative")$meanRating,
       paired = TRUE)
cohen.d(d = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Neutral")$meanRating,
       f = subset(word.means.valence, IncentiveType=="Liquid" & Word_Valence=="Negative" & Valence=="Negative")$meanRating,
       paired = TRUE)
```
...
p.motivation.sf.lf<-ggplot(data = data.motivation.sf.lf.sum, aes(x = Valence, y = rating, fill = money)) +
  ...
  xlab("Feedback Valence") + ylab("Motivation Rating") + ggtitle("Self-Reported Motivation") +
  scale_fill_discrete(name="Monetary\nReward") +
  theme(plot.title = element_text(hjust = 0.5), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"), 
    axis.title=element_text(size=12,face = "bold"),
    axis.text=element_text(size=14),
...
mot.tab.E1 <- data.motivation.sf.lf.sum %>% select(Valence, money, IncentiveType, rating, sd) %>%
  ...
  kable(., format = "html", digits = 3, booktabs = T,
        col.names = c("Feedback Valence", "Monetary Reward", rep(c("Mean","SD"), times = 2)),
...
p.liking.sf.lf<-ggplot(data = data.liking.sf.lf.sum, aes(x = Valence, y = rating, fill = money)) +
  ...
  geom_errorbar(position=position_dodge(.9),
                mapping = aes(ymin=rating-ci, ymax=rating+ci), width=.2) +
  xlab("Feedback Valence") + ylab("Liking Rating") + ggtitle("Self-Reported Liking") +
  scale_fill_discrete(name="Monetary\nReward") +
  theme(plot.title = element_text(hjust = 0.5), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"), 
    axis.title=element_text(size=12,face = "bold"),
    axis.text=element_text(size=14),
...
data.val.inc.mon.RTmeans = group_by(data, subID, Valence,IncentiveType,moneyreward) %>% filter(!is.na(subRewarded) & ACC==1) %>%
  dplyr::summarise(n=n(), meanRT = mean(RT, na.rm = T), sdRR = sd(RT, na.rm = T))
...
t.test(x = subset(data.incentive.means, IncentiveType=="Social")$meanRR,
       ...
       paired = TRUE)
cohen.d(d = subset(data.incentive.means, IncentiveType=="Social")$meanRR,
       f = subset(data.incentive.means, IncentiveType=="Liquid")$meanRR,
       paired = TRUE)
...
t.test(x = subset(data.incentive.means, IncentiveType=="Social")$meanACC,
       ...
       paired = TRUE)
cohen.d(d = subset(data.incentive.means, IncentiveType=="Social")$meanACC,
       f = subset(data.incentive.means, IncentiveType=="Liquid")$meanACC,
       paired = TRUE)
...
t.test(x = subset(data.incentive.means, IncentiveType=="Social")$meanErr,
       ...
       paired = TRUE)
cohen.d(d = subset(data.incentive.means, IncentiveType=="Social")$meanErr,
       f = subset(data.incentive.means, IncentiveType=="Liquid")$meanErr,
       paired = TRUE)
```

###Summary Table of Reward Rate, Accuracy, Error Rate, and RT

``` {r RRSum2, message = F, warning =  F}
#Summary Table of Rewrard Rate, Accuracy, Error Rate, and RT
RR.summary.2<-summarySEwithin2(data = data.val.inc.mon.means, measurevar = "meanRR", withinvars = c("Valence","IncentiveType","moneyreward"), idvar = "subID")
ACC.summary <-summarySEwithin2(data = data.val.inc.mon.means, measurevar = "meanACC", withinvars = c("Valence","IncentiveType","moneyreward"), idvar = "subID") %>%
  rename("ACC_mean" =  meanACC,
         "ACC_sd" = sd) %>%
  select(ACC_mean, ACC_sd)

Err.summary <- summarySEwithin2(data = data.val.inc.mon.means, measurevar = "meanErr", withinvars = c("Valence","IncentiveType","moneyreward"), idvar = "subID") %>%
   rename("Err_mean" = meanErr,
         "Err_sd" = sd) %>%
  select(Err_mean, Err_sd)

RT.summary <- summarySEwithin2(data = data.val.inc.mon.RTmeans, measurevar = "meanRT", withinvars = c("Valence","IncentiveType","moneyreward"), idvar = "subID") %>%
   rename("RT_mean" = meanRT,
         "RT_sd" = sd) %>%
  select(RT_mean, RT_sd)

summary.table.2 <- cbind(RR.summary.2,ACC.summary, Err.summary, RT.summary) %>%
  rename("RR_mean" = meanRR,
         "RR_sd" = sd) %>%
  select(-c("N","meanRRNormed","se","ci"))

###Summary Table: Reward Rate, Accuracy, Error Rate, RT
summary.table.2$moneyreward <- factor(summary.table.2$moneyreward, levels = c("$","$$","$$$$"), labels = c("Low","Medium","High"))
tab.RR.sum.2 <- summary.table.2 %>% 
  pivot_wider(names_from = "IncentiveType", values_from = c("RR_mean","RR_sd","ACC_mean","ACC_sd","Err_mean","Err_sd",
                                                            "RT_mean","RT_sd")) %>%
  select(Valence, moneyreward, RR_mean_Liquid, RR_sd_Liquid, ACC_mean_Liquid, ACC_sd_Liquid, Err_mean_Liquid, Err_sd_Liquid, RT_mean_Liquid, RT_sd_Liquid,
         RR_mean_Social, RR_sd_Social, ACC_mean_Social, ACC_sd_Social, Err_mean_Social, Err_sd_Social, RT_mean_Social, RT_sd_Social)  %>%
...
###ANOVA Tables: Feedback Valence x Incentive Type x Monetary Reward (Experiment II)
...
d.soc.anova.E2 <- social.anova.E2$ANOVA %>%
  mutate(Effect = factor(Effect, levels = c("Valence","moneyreward","Valence:moneyreward"),
                         labels = c("Feedback Valence","Monetary Reward","Feedback Valence x Monetary Reward"))) %>%
  rename("Generalized Eta Squared" = ges)
tab.soc.E2 <- kable(d.soc.anova.E2, digits = 3, caption = "Social Feedback ANOVA (Feedback Valence x Monetary Reward)") %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
tab.soc.E2
...
d.liq.anova.E2 <-liquid.anova.E2$ANOVA %>%
  mutate(Effect = factor(Effect, levels = c("Valence","moneyreward","Valence:moneyreward"),
                         labels = c("Feedback Valence","Monetary Reward","Feedback Valence x Monetary Reward"))) %>%
  rename("Generalized Eta Squared" = ges)
tab.liq.E2 <- kable(d.liq.anova.E2, digits = 3, caption = "Liquid Feedback ANOVA (Feedback Valence x Monetary Reward)") %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
tab.liq.E2
```
