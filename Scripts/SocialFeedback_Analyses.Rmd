...
BISBAS.means <- BISBAS %>%
  dplyr::summarise(mean_Reward = mean(BAS_Reward), sd_Reward = sd(BAS_Reward), mean_Fun = mean(BAS_Fun), sd_Fun = sd(BAS_Fun), 
                   mean_Drive = mean(BAS_Reward), sd_Drive = sd(BAS_Drive), mean_BIS = mean(BIS), sd_BIS = sd(BIS)) %>%
...
DARS.means <- DARS %>%
  dplyr::summarise(mean_Hobby = mean(DARS_Hobby), sd_Hobby = sd(DARS_Hobby), mean_Food = mean(DARS_Food), sd_Food = sd(DARS_Food),
                   mean_Social = mean(DARS_Social), sd_Social = sd(DARS_Social), mean_Sensory = mean(DARS_Sensory), sd_Sensory = sd(DARS_Sensory)) %>%
...
p.Session.RR.ind.2 <- ggplot(data = m.RR.sum, aes(x=IncentiveType, y=meanRR)) +
           geom_bar(stat="identity", position=position_dodge()) + 
  geom_errorbar(data = m.RR.sum, position=position_dodge(width=0.9), aes(ymin=meanRR-ci, ymax=meanRR+ci), width=.2) +
  geom_point(data = incentive.means, aes(x=IncentiveType, y=meanRR),
             stat="identity", position=position_jitter(w=.3), alpha=0.5) +
...
DARS.E2.means <- DARS.E2 %>%
  dplyr::summarise(mean_Hobby = mean(DARS_Hobby), sd_Hobby = sd(DARS_Hobby), mean_Food = mean(DARS_FoodDrink), sd_Food = sd(DARS_FoodDrink),
                   mean_Social = mean(DARS_SocialActivity), sd_Social = sd(DARS_SocialActivity), mean_Sensory = mean(DARS_SensoryExp), sd_Sensory = sd(DARS_SensoryExp)) %>%
  kable(format = "html") %>%
    kable_styling(full_width = F)
