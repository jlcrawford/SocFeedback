...
rm(list=ls())
# load libraries
library(kableExtra); library(ez); library(knitr); library(RColorBrewer); library(tidyverse); source("summarySEwithin2.R")
# summarySEwithin2 function 
source("https://github.com/jlcrawford/SocFeedback/blob/master/summarySEwithin2.R")
```
...
``` {r RR_SFLF, echo = TRUE}
#Import Data
data.path.sf <- "https://github.com/jlcrawford/SocFeedback/blob/master/Experiment1/Data/SF_merged_FINAL.csv"
data.path.lf <- "https://github.com/jlcrawford/SocFeedback/blob/master/Experiment1/Data/LF_merged_FINAL.csv"

data.reward.sf <-read.csv(data.path.sf, header = TRUE) %>% select(-X) %>%
...
data.reward.lf <-read.csv(data.path.lf, header = TRUE) %>% select(-X) %>%
    select(Subject, Trial, Condition, Target.RT, CriterionRT, Target.ACC, Target.RESP, Session, CorrectResponse, Reward, Procedure.Trial.) %>%
...
t.test(x = subset(val.inc.mon.means, IncentiveType=="Social")$meanRR,
       y = subset(val.inc.mon.means, IncentiveType=="Liquid")$meanRR,
       paired = TRUE)
S.mean.RR = mean(subset(val.inc.mon.means, IncentiveType=="Social")$meanRR)
L.mean.RR = mean(subset(val.inc.mon.means, IncentiveType=="Liquid")$meanRR)

#Accuracy Difference Across Social and Liquid Feedback
t.test(x = subset(val.inc.mon.means, IncentiveType=="Social")$meanACC,
       y = subset(val.inc.mon.means, IncentiveType=="Liquid")$meanACC,
       paired = TRUE)
S.mean.ACC = mean(subset(val.inc.mon.means, IncentiveType=="Social")$meanACC)
L.mean.ACC = mean(subset(val.inc.mon.means, IncentiveType=="Liquid")$meanACC)

#Error Rate Difference Across Social and Liquid Feedback
t.test(x = subset(val.inc.mon.means, IncentiveType=="Social")$meanErr,
       y = subset(val.inc.mon.means, IncentiveType=="Liquid")$meanErr,
       paired = TRUE)
S.mean.Err = mean(subset(val.inc.mon.means, IncentiveType=="Social")$meanErr)
L.mean.Err = mean(subset(val.inc.mon.means, IncentiveType=="Liquid")$meanErr)
...
affect.sf <- read.csv('DynamStim_v2_FINAL.csv') %>% select(Subject, Trial, Condition, Feeling, Ratings.RESP, Ratings.RT, Procedure.Trial.) %>%
  mutate(IncentiveType="Social", 
         Valence=factor(Condition, levels=c("P","T","N"), labels=c("Positive","Neutral","Negative"))) %>% 
  filter(Procedure.Trial. == "RatingsTask")

affect.lf <- read.csv('LF_merged_FINAL.csv') %>% select(Subject, Trial, Condition, Feeling, Ratings.RESP, Ratings.RT, Procedure.Trial.) %>% 
  mutate(IncentiveType="Liquid",
         Valence=factor(Condition, levels=c("J","N","S"), labels=c("Positive","Neutral","Negative"))) %>%
  filter(Procedure.Trial. == "RatingsTask")
...
kable(liquid.affect.anova)
liquid.affect.anova
```

##Self-Report Ratings (Supplemental Material)

``` {r SelfReport}
# Import Self-Report Data
data.path.selfreport < "https://github.com/jlcrawford/SocFeedback/blob/master/Experiment1/Data/SFLF_SelfReport.csv"
data.post.task <- read.csv(data.path.selfreport, header = TRUE) %>% select(-X)
...
p.motivation.sf.lf<-ggplot(data = data.motivation.sf.lf.sum, aes(x = Valence, y = rating, fill = money)) +
  ...
  xlab("Feedback") + ylab("Motivation Rating") + ggtitle("Self-Reported Moitvation") +
  scale_fill_discrete(name="Monetary\nReward") +
  theme(plot.title = element_text(hjust = 0.5), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"), 
    axis.title=element_text(size=12,face = "bold"),
    axis.text=element_text(size=14),
...
kable(social.anova.motive.E1)
social.anova.motive.E1
...
kable(liquid.anova.motive.E1)
liquid.anova.motive.E1
...
kable(social.anova.liking.E1)
social.anova.liking.E1
...
kable(liquid.anova.liking.E1)
liquid.anova.liking.E1

#BISBAS
BISBAS <- data.post.task %>% select(Subject, starts_with("bisbas"))
...
data.path <- "https://github.com/jlcrawford/SocFeedback/blob/master/Experiment2/Data/Data_Reward.csv"
baseline.path<-"https://github.com/jlcrawford/SocFeedback/blob/master/Experiment2/Data/Data_Baseline.csv"
indiv.path <- "https://github.com/jlcrawford/SocFeedback/blob/master/Experiment2/Data/Data_Indiv_Composite.csv"
ratings.liq.path <- "https://github.com/jlcrawford/SocFeedback/blob/master/Experiment2/Data/Data_Ratings_Liquid.csv"
ratings.soc.path <- "https://github.com/jlcrawford/SocFeedback/blob/master/Experiment2/Data/Data_Ratings_Social.csv"
```
...
#Reward Rate Difference Across Social and Liquid Feedback
t.test(x = subset(data.val.inc.mon.means, IncentiveType=="Social")$meanRR,
       y = subset(data.val.inc.mon.means, IncentiveType=="Liquid")$meanRR,
       paired = TRUE)
S.mean.RR.2 = mean(subset(data.val.inc.mon.means, IncentiveType=="Social")$meanRR)
L.mean.RR.2 = mean(subset(data.val.inc.mon.means, IncentiveType=="Liquid")$meanRR)

#Accuracy Difference Across Social and Liquid Feedback
t.test(x = subset(data.val.inc.mon.means, IncentiveType=="Social")$meanACC,
       y = subset(data.val.inc.mon.means, IncentiveType=="Liquid")$meanACC,
       paired = TRUE)
S.mean.ACC.2 = mean(subset(data.val.inc.mon.means, IncentiveType=="Social")$meanACC)
L.mean.ACC.2 = mean(subset(data.val.inc.mon.means, IncentiveType=="Liquid")$meanACC)

#Error Rate Difference Across Social and Liquid Feedback
t.test(x = subset(data.val.inc.mon.means, IncentiveType=="Social")$meanErr,
       y = subset(data.val.inc.mon.means, IncentiveType=="Liquid")$meanErr,
       paired = TRUE)
S.mean.Err.2 = mean(subset(data.val.inc.mon.means, IncentiveType=="Social")$meanErr)
L.mean.Err.2 = mean(subset(data.val.inc.mon.means, IncentiveType=="Liquid")$meanErr)
...
##Self-Report Data (Supplemental Material)
...
data.liq.valence<-gather(data = data.liq.valence, key = valence, value = liking_rating, 
  ...
  mutate(Valence = factor(liquid, levels = c("Juice","Neutral","Salt"), 
                          labels = c("appetitive","neutral","aversive")))
